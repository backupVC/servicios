...
\begin{parts}
\part
(2 ptos) Un cierre (\emph{clousure}) que pase como argumentos un \emph{array} de objetos persona con los siguientes campos (iguales que los datos del fichero de \emph{arrays} de \emph{json} que se te proporiciona):
\begin{itemize}
\item first\_name
\item last\_name
\item email
...
\item Nos de un array de objetos persona con una edad igual a superior a una edad que se pase por parámetro.
\end{itemize}
(2 ptos) Un programa denominada \emph{main.js} que lea el fichero \emph{examen1A.json} que consta de un array de \emph{json} sobre datos de personas. Para la lectura solo tienes que usar el método \emph{readFile} con la correspondiente \emph{callback} y parsear los datos solamente con \emph{JSON.parse}. (No hace falta liar tanto código como en el ejemplo realizado en clase con un fichero de tipo \emph{CSV}).\\
...
\part Crea una base de datos en \emph{mongodb} denominada \emph{examen}. Importa los datos del fichero \emph{examen1A.json} a una colección de dicha base de datos, llama a dicha colección \emph{persons}
...
\begin{itemize}
\item Obtener un documento de la colección dado el email
\item Crear un documento nuevo
\item Borrar un documento dado el email.
\end{itemize}
